...
def define_args():
    ...
    parser.add_argument('--parent_saved_dir', default='outputs_paper',type=str)
...
def train_model(args, start_epoch,n_epochs, iteration, best_corr, training_generator, validation_generator, model):
    model_dir = 'outputs_paper/' + args.brain_region + '_' + args.readout
    ...
    logging.basicConfig(filename='outputs_paper/' + args.brain_region + '_' + args.readout + '.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def evaluate_trained_model(args, model, validation_generator, test_generator):
    ...
    if args.readout == 'spatial_linear' or 'linear_ridge' in args.readout:
        true, preds = compute_predictions_spatial(validation_generator, model)
    if args.readout == 'semantic_transformer':
        true, preds = compute_predictions_semantic(validation_generator, model)
    val_corr = compute_scores_evaluation(true, preds)
    ...
    val_corr_norm = val_corr/noise_ceiling
    ...
    np.save('metrics/' + args.brain_region + '_' + args.readout + '.npy', np.array(test_corr))
    ...
    val_corr, val_corr_norm = np.nanmean(val_corr), np.nanmean(val_corr_norm)
    with open(evaluation_dir+'/metrics_' + args.brain_region + '_' + args.readout + '.txt', 'w') as file:
        file.write('val_corr :: '+str(val_corr)+'\n'+'val_corr_norm :: '+str(val_corr_norm) + '\n')
...
if __name__ == "__main__":
    ...
    if args.readout == 'gaussian2d':
        ...
        readout = RidgeRegression(core(torch.randn(1, 3, 112, 112)).view(1, -1).size()[1], n_neurons, alpha=args.alpha)  
    ...
    parent_saved_dir = os.path.join(args.parent_saved_dir,args.brain_region)
